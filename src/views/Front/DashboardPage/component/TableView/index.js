...
const TableView = (props) => {
      ...
      return val.map((item) => {
        ...
        if (needChangeColor.includes(item.key)) {
          item.render = (text) => {
            ...
            return <span style={{ color }}>{`${textNum}%`}</span>
          }
          // price 同涨幅颜色值
  ...
  useEffect(() => {
    const emitResize = throttle(() => {
      ...
      if (!tableDiv) return
      const theadHeight = window
        .getComputedStyle(
          document.querySelector(`#${props.widget.id} .ant-table-thead`),
        )
        .height.slice(0, -2)
    ...
    document.getElementById(props.widget.id) &&
      resizeObserver.observe(document.getElementById(props.widget.id))
    return () => {
      document.getElementById(props.widget.id) &&
        resizeObserver.unobserve(document.getElementById(props.widget.id))
    }
  }, [props.widget])
...
const mapStateToProps = (state, ownProps) => {
  ...
  if (ownProps.widget && ownProps.widget.id) {
    businessData =
      state.dashboardStore.chartsData[ownProps.widget.id] &&
      state.dashboardStore.chartsData[ownProps.widget.id].data
  }
  return {
    businessData,
  }
