...
class GridView extends PureComponent {
  ...
  showFullScreen(id) {
    ...
    gridWrapper.style.width = sideBarWidth
    ...
    const minContent = () => {
      return (
        <>
          ...
          <div class="action-item-wrapper">最小化</div>
        </>
      )
    }
    if (minWrapper) {
  ...
  getChartDom = () => {
    return this.state.widgets.map((widget, index) => {
      if (
        this.props.chartsData[widget.id] &&
        this.props.chartsData[widget.id].chart
      ) {
      ...
      if (widget.type === 'CHART') {
        ...
        component = <MarkdownView widget={widget} />
      } else if (widget.type === 'REALVIEW') {
        component = <RealView widget={widget} />
      } else if (widget.type === 'FEED') {
        component = <Feed widget={widget} />
      } else if (widget.type === 'EDITFEED') {
        component = <EditFeed widget={widget} dashboardId={this.props.id} />
      } else if (widget.type === 'TABS') {
        component = <TabsView widget={widget} dashboardId={this.props.id} />
        ...
        component = (
          <PageHeader
            ...
            backIcon={
              <div
                style={{ width: '3px', height: '25px', background: '#1890ff' }}
              ></div>
            }
          />
        )
        ...
        component = <Filter widget={widget} dashboardId={this.props.id} />
      } else {
        component = <div key={widget.i}>{widget.i}</div>
      }
      return (
        <div
          key={widget.i}
          ...
          data-static={widget.static}
        >
          <div className="grid-wrapper">
            {widget.type !== 'MARKDOWN' &&
              widget.chartStyle &&
              widget.chartStyle.chart &&
              widget.chartStyle.chart.title && (
            ...
            {widget.type === 'CHART' && (
              <div className="remove">
                ...
                <div
                  ...
                  onClick={this.showFullScreen.bind(this, widget.id)}
                >
                  最大化
                </div>
              </div>
  ...
  render() {
    return (
      <div>
        <Anchor links={this.props.anchorList} />
        <div
          className="grid-con-wrap"
          style={this.props.isTabs ? { padding: 0 } : { padding: 20 }}
        >
...
const mapStateToProps = (state, ownProps) => {
  ...
  if (ownProps.location) {
    ...
    widgets =
      state.dashboardStore.boardGridOrigin[id] &&
      state.dashboardStore.boardGridOrigin[id].widgets
    disable =
      state.dashboardStore.boardGridOrigin[id] &&
      state.dashboardStore.boardGridOrigin[id].disable
  } else {
